...
// 为什么要使用动态链接？
//
//其实动态链接技术出现的最重要目的，便是为了解决静态链接具有的一些明显缺点。试想，假设一个应用程序依赖于多个第三方模块提供的函数实现，而这些模块均以静态库（包含有多个目标文件）的方式提供。那么，每次想要使用它们的最新版本时，都需要显式地将程序与它们重新进行链接。对于大多数普通的应用使用者来说，这个过程所花费的成本当然是无法接受的。
//
...
//
//能够使用动态链接加载的库被称为“共享库（Shared Library）”。在 Linux 中，这类库文件通常以 “.so” 后缀结尾。

// 使用共享库
//
//动态库本身也是 ELF 格式的一种具体文件类型，它对应着 elf.h 中的宏常量 ET_DYN。
//
//这里将把 sum.c 文件编译成动态库，并让 main.c 对应的应用程序使用。整个过程可以分为如下几步：
//
//1. 使用命令 `gcc sum.c -shared -fPIC -o libsum.so` 将文件 sum.c 编译成名为 libsum.so 的动态库文件。这一步中使用的参数 “-shared” 表明创建一个动态库；参数 “-fPIC” 表明生成“位置无关代码”。
//2. 使用命令 `gcc -o main main.c -lsum -L.` 编译应用程序。这里将 main.c 与第一步生成的 libsum.so 共享库放在一起编译。命令中，参数 “-L.” 可用于为编译器指定更多的共享库查找目录，这里为其添加了 libsum.so 的所在目录；参数 “-l” 则用于指定需要参与编译的共享库，通过指定名称 “sum”，编译器会自动使用搜索到的，合法的 libsum.so 文件。
...
//4. 使用命令 `./main` 运行程序。
//
//需要注意的是，除了可以使用上述第三步介绍的“修改 LD_LIBRARY_PATH 变量”的方式来指定共享库的运行时查找目录外，还可以使用 [rpath](<https://en.wikipedia.org/wiki/Rpath>) 和 [ldconfig](<https://man7.org/linux/man-pages/man8/ldconfig.8.html>) 这两种方式。它们分别通过“将动态库所在路径嵌入到可执行文件”，以及“将共享库安装到当前系统可访问的全局环境中”这两种方式，使得对应的共享库可以顺利地被动态链接器查找。可以直接点击对应的链接，来了解更多信息。
//
//而为了让共享库真正地做到“可以被多个进程共享”，便需要让它的代码成为“位置无关代码”。下面来看看这个概念。

// 位置无关代码
//
//位置无关代码（Position Independent Code，PIC）是一类特殊的机器代码，这些代码在使用时，可以被放置在每个进程 VAS 中的任意位置，而无需链接器对它内部引用的地址进行重定位。大多数现代 C 编译器在编译源代码时，均会默认产生这种 PIC 代码，而无需用户显式指定。当然，为了以防万一也可以通过添加 “-fPIC” 等参数的方式来明确指出。
//
//通常来说可以将模块（可以理解为独立的应用程序，或共享库）之间的数据引用分为四种方式：
//
//- 模块内部的函数调用；
//- 模块内部的数据访问；
//- 模块之间的函数调用；
...
//
//PIC 的出现使得共享库代码可以做到真正地被多个进程复用，它利用了一个很简单的思想，即“将易变的部分抽离到进程独享的可修改内存中”。而为了做到这一点，编译器需要为各个模块添加额外的 Section 结构，这就是“全局偏移表”。
...
//
//模块在被编译时，其 Text Segment 与 GOT 之间的相对距离是能够计算出来的。因此，编译器可以利用这一点，来让代码直接引用 GOT 中的某个表项。同时，编译器还会为这些表项生成相应的重定位记录。这样，当程序被加载进内存时，动态链接器就可以根据实际情况，通过修正 GOT 表项中的值，来做到间接修正代码中对应符号的实际引用地址。可以通过下图来直观地感受这个流程：
//
//![图片](.img/got-text.png)
//
...
// 过程链接表
//
//虽然可以让动态链接器在程序加载时，将其代码中使用到的所有外部符号地址，更新在相应的 GOT 表项中，但当程序依赖的外部符号越来越多时，重定位的成本也会越来越高。而这便会导致程序初次运行时的“启动延迟”逐渐变大，甚至影响到程序正常功能的运作。为了解决这个问题，编译器为模块另外添加了名为“过程链接表（Procedure Linkage Table，PLT）”的 Section 结构。该表将协同 GOT，一起进行针对函数符号地址的“延迟绑定”。
//
...
//
//接下来详细看看延迟绑定的具体执行过程。这里以上面“使用共享库”示例，共享库里 sum 函数的调用过程为例来进行介绍。可以先看看下面的图片，对整体流程有个大致的感知，然后具体来看每个步骤。
//
//![图片](.img/plt-text.png)
//
//sum 函数的初次调用过程可以分为四步：
...
//
//以上便是 sum 函数初次进行地址延迟绑定，以及再次访问时的整体流程，动态链接的基本实现方式，下面来看看基于此进行的加载时链接与运行时链接这两者的主要区别。

// 加载时链接
//
//实际上，加载时链接作为动态链接的一种具体类型，便是基于 GOT 与 PLT 两个表结构进行的。它的一个最主要特征是，**动态链接器进行的符号重定位过程发生在程序代码被真正执行之前**。而为了做到这一点，操作系统执行应用程序的具体步骤也发生了改变。
//
...
//这段代码的逻辑十分简单。通过“运行时链接”的方式，在程序的运行过程中从共享库文件 libm.so.6 内部加载了函数 cos。而在程序最后，使用实参 2.0 调用了这个函数，并打印出了执行结果。
//
//这里，函数 dlopen 用于打开一个指定的共享库。通过它的第二个参数，能够指定符号重定位的具体执行方式。这里的 RTLD_LAZY 表示延迟绑定，即动态链接器仅会在特定函数被调用时，才对其使用到的相关符号进行解析。
//
//函数 dlsym 则用于从一个打开的共享库实例中获取某个具体符号的地址。而在此之后，便能够以函数指针的形式对它进行调用。最后，当共享库使用完毕，通过 dlclose 函数，可以减少共享库实例的被引用次数。而当该次数变为 0，且共享库对象中的符号没有被其他对象引用时，该共享库对应内存便会从当前进程的 VAS 中被卸载，卸载的具体时机则由操作系统决定。
//
//在上述整个流程中，可以使用 dlerror 函数，随时获取 dlopen API 函数在执行过程中产生的错误诊断信息。
...
//Linux 共享库使用的 soname 机制?
//
//在Linux中，soname（共享对象名称）机制是用于在共享库中实现版本控制和动态链接的一种机制。soname包括两个部分：实际的库名称和版本号。
//
//实际的库名称： 是库的基本名称，通常以lib开头，后跟库的名称。例如，一个名为libexample.so的共享库的实际库名称是example。
//
//版本号： 包括主版本号、次版本号和修订号。版本号的格式通常为<major>.<minor>.<revision>，例如1.2.3。在soname中，版本号被添加到库的名称中，形成完整的soname。
//
//示例：
//假设有一个共享库libexample.so，其实际库名称是example，版本号是1.2.3。
//
//soname示例： libexample.so.1.2.3
//
//这个soname表明了以下几个重要信息：
//
//实际库名称是example。
...
//soname的作用：
//
//1. 在库的文件名中嵌入版本信息： soname机制的一个关键作用是在库的文件名中嵌入版本信息，以便区分不同版本的库。
//
//2. 版本控制： soname允许在系统中同时存在不同版本的共享库，以便应用程序可以链接到其所需的特定版本。这有助于保持对旧版本的兼容性，同时允许新应用程序使用新版本的库。
//
//3. 动态链接： soname也用于创建符号链接，动态链接器使用这些链接来查找正确的共享库。通常，存在一个不带版本号的符号链接，指向包含完整版本号的库文件。
//
//soname的使用示例：
//
//1. 编译共享库时指定soname：
//
//gcc -shared -o libexample.so.1.2.3 example.c -Wl,-soname,libexample.so.1
//
//这个命令将创建一个名为libexample.so.1.2.3的共享库文件，并将libexample.so.1作为其soname。
//
//2. 创建符号链接：
//
//ln -s libexample.so.1.2.3 libexample.so.1
//
//这个命令创建了一个名为libexample.so.1的符号链接，指向libexample.so.1.2.3。
//
//3. 运行时链接：
//
//在运行时，动态链接器会使用soname来查找所需的共享库。程序链接时会指定soname，而动态链接器将使用这个soname来查找正确的库文件。
//
//通过soname机制，Linux系统能够更好地管理和维护共享库的版本，以确保应用程序在运行时可以正确地链接到所需的库版本。
