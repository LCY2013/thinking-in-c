...
//
//先从整体的视角看看内存在计算机系统中的作用，然后再探究进程是如何在操作系统的控制下与计算机内存交互的。
...
//
//将这个例子类比到计算机，会发现两者之间有着类似的行为模式。每一个程序的机器代码（事务）在可以被 CPU（负责人）正常执行前，操作系统都需要先将它们从磁盘（档案柜）“搬移”到内存（书桌）中。而随着程序的不断运行，那些被经常访问的数据便会被存放到较高级别的缓存（较近位置）中。相反，不常用的数据则会被存放在较低级别的缓存（较远位置），甚至驻留在内存中。
//
//但是，对计算机而言，程序与内存之间的交互细节远比“办公室日常”要复杂得多。现代操作系统会同时执行十几个甚至几十个程序。因此，如何从有限的内存中合理地为它们分配所需资源，并同时兼顾安全性、高效性，便成为需要考虑的首要问题。
//
//现代计算机通过名为“虚拟内存”的机制，做到了这一点。下面来进一步看看这个机制的具体工作原理。

// 虚拟内存机制
//
//顾名思义，虚拟内存（Virtual Memory）对应于物理内存（Physical Memory）。其中，前者是由操作系统抽象出来的一个概念，它在后者的基础之上进行了一层抽象，以帮助运行于其上的应用程序合理地分配内存，并管理内存使用。
//
//因此，如下面的代码所示，在应用程序中打印出的各种指针值，它们实际上都对应于虚拟内存中的某个地址，而非实际的物理内存地址（Physical Address，PA）。这些地址被称为“虚拟地址（Virtual Address，VA）”。所有程序可以使用的虚拟地址则构成了虚拟地址空间（VAS）。
//
//```bash
//#include <stdio.h>
//int main(void) {
...
//```
//
//CPU 在访问内存中的数据时，会借助其芯片上名为“内存管理单元（MMU）”的硬件，首先将虚拟地址动态翻译为对应的物理地址，然后再进行实际的数据获取。可以通过下图来直观地理解这个过程。
//
//![图片](.img/mmu_virtual_memory.png)
//
...
//
//而通过下面这行命令，便可以查看某个运行进程的 VAS 布局情况。注意，其中的 “<pid>” 需要被替换为进程对应的 ID。</pid>
//
//```bash
//cat /proc/<pid>/maps
//```
//
//这里开头处的那个 C 程序的 ID 替换到上述命令中。在运行该命令后，可以得到如下图所示的输出结果。
//
//![图片](.img/virtual_memory_layout.png)
//
//这里，命令按照地址由低到高的顺序打印出了进程 VAS 内，每一块已经被占用的连续虚拟内存地址，对应的映射信息。如最右侧一列所示，这些内存中的内容或是来自于某个具体文件（/www/workspace/main），或被用作其他用途（[heap]）。

// VAS 中的数据布局
//
//将上图中的信息进行归类，可以得到如下图所示的 Linux 进程在 VAS 内的统一数据布局结构。这里根据类别将不同的数据用不同的颜色进行了标注。并且为了方便找到这两个图之间的内存段对应关系，将上图中的一些关键性地址信息也选择性地标注在了下图中。
//
//![图片](.img/virtual_memory_layout2.png)
//
...
//- **堆（Heap）**：关于堆内存，随着动态数据的不断产生，它将向 VAS 的高地址方向不断增长。
//- **共享库数据**：这部分内存中包含有与各类 .so 共享库相关的数据，程序会在运行时通过动态链接器来完成对它们的加载和处理。
//- **栈（Stack）**：关于栈内存，随着各种局部变量的不断产生，它将向 VAS 的低地址方向不断增长。
...
//
//可以看到，得益于虚拟内存机制的抽象，进程可以使用完全统一、独立的内存数据布局，而不用考虑这些数据在真实物理内存中的具体存储细节。那么，虚拟内存机制究竟是如何对物理内存进行管理的呢？接下来具体看看。
...
//
//这里可以先暂缓脚步，通过下图来直观地理解 CPU、MMU、页表、物理内存，以及磁盘五者之间的协作关系。
//
//![图片](.img/cpu-mmu-physical-memory.png)
//
//页表隔离了进程的 VAS 与物理内存，使得两者之间的映射关系变得更加自由。而在这种方式下，当不同进程使用不同页表维护其各自 VAS 中虚拟页的映射时，多个进程之间便可做到真正的数据共享。而“动态链接”技术便以此为基础。不仅如此，独立的 VAS 与页表也使得进程之间的私有内存不会被相互访问。
//
...
//
//按照树的形式展开，每个一级页表也都对应着一个独立的二级页表，二级页表中的每一个 PTE 则负责映射当前一级页表的 4MiB 的片中的某个 4KiB 的块。当一级页表查询完毕后，MMU 便可得到指向目标二级页表的地址。通过该地址，再联合虚拟地址中的另一部分虚拟页号信息，它便可找到目标数据所在物理内存的具体页。最后，结合虚拟地址中的页偏移信息，目标数据的最终物理地址便可被成功地“转换”出来。可以通过下图来进一步理解上述流程：
//
//![图片](.img/cpu-mmu-physical-memory-multi-level-page-table.png)
//
...
//
//TLB 属于 MMU 的一部分，它可以加快 MMU 根据虚拟地址查询 PTE 的过程。可以将 TLB 理解为一个简单的具有 N 行 M 列的矩阵，MMU 会从对应虚拟地址中提取出用于查询表项的 TLB 索引与 TLB 标记。这两个值可以联合起来使用，并定位到 TLB 中的一个具体单元格。而此时，若该单元格内有值，则 MMU 可以直接使用该值，来与虚拟地址中的其他信息一起组成最终的物理地址。否则，MMU 仍然需要通过逐级查询页表的方式来获取目标页的物理地址。

// 共享对象与私有对象
//
//借助虚拟内存机制，不同进程之间可以共享物理内存上的同一段数据。
//
//这些数据在物理内存中实际存放时，可能并不是连续的。而借助于页表实现的“虚拟页与物理页映射关系分离”，可以确保 CPU 能够按照连续的方式来使用这些数据。
//
...
//为什么 Linux 进程 VAS 从地址 0x0 开始，直到 0x400000 的低地址段，没有存放任何数据？
//
//在Linux中，每个进程的虚拟地址空间从0x0开始，直到0x400000（这是默认情况下，具体数值可能会根据操作系统和架构而有所不同）。这个低地址段通常被保留，主要有以下几个原因：
//
//空指针引用检测： 将低地址段保留为空，有助于检测空指针引用。如果一个指针被错误地设置为0，那么任何试图访问这个地址的操作都将触发段错误（Segmentation Fault），从而使程序员能够更容易地发现和修复空指针引用的错误。
//
//内核空间和用户空间划分： 通常，低地址段是用于内核空间的，而高地址段是用于用户空间的。这种划分使得内核和用户空间能够独立运行，提高系统的稳定性和安全性。
//
//保留用途： 一些特殊的用途或保留给操作系统使用的区域可能存在于低地址段，这可能包括内核代码、页表等。
//
//请注意，这个地址分布是默认情况下的通用做法，实际上可以通过操作系统的配置进行调整。在不同的情况下，这个低地址段的大小和用途可能会有所不同。
